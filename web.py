...
if st.session_state["use_rag"]:
    ...
    if uploaded_files:
        ...
        with st.sidebar.expander("업로드한 PDF 파일 보기/다운로드", expanded=False):
            ...
            for file_name, file_data in st.session_state["uploaded_file_data"].items():
                st.download_button(
                    label=f"📥 {file_name} 다운로드",
                    data=file_data,
                    file_name=file_name,
                    mime="application/pdf",
                    key=f"download_orig_{file_name}"
                )
                    ...
                    for doc_idx, src_doc in enumerate(msg["sources"]):
                        ...
                        if os.path.exists(src_path):
                            ...
                            if 0 <= page_number < len(reader.pages):
                                ...
                                temp_dir = tempfile.gettempdir()
                                snippet_pdf_path = os.path.join(temp_dir, f"{base_name}-{page_num}p.pdf")
                                ...
                                st.download_button(
                                    label=f"📥 해당 페이지({page_num+1}p) 발췌 PDF 다운로드",
                                    data=snippet_pdf_data,
                                    file_name= f"{base_name}-{page_num+1}p.pdf",
                                    mime="application/pdf",
                                    key=f"snippet_download_btn_{page_number}_{doc_idx}"
                                )
                            else:
                            ...
                            st.download_button(
                                ...
                                file_name=file_name,
                                mime="application/pdf",
                                key=f"download_btn_{i}_{doc_idx}"
                            )
                        else:
